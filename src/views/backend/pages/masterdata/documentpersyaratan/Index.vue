<template>
  <div class="home pa-6">
    ...
    <v-flex class="pb-5 pl-7">
      <div class="grey--text mb-2">Seluruh Daftar Permohonan Dokumen Persyaratan Yang Tersedia</div>
    </v-flex>
      ...
      <v-col cols="12">
        <v-card class="animated animate__fadeInUp">
              ...
              <template v-slot:activator="{ on }">
                ...
                >
                  <v-icon
                    ...
                    @click="$router.push({name:'master-jenis-document'})"
          ...
          >
            ...
            <template v-slot:item.wajib="{ value }">
              <v-chip
                :color="value.color"
                small
              >{{ value.text }}</v-chip>
              ...
              <v-row>
                <v-col cols="3">
                  <v-switch
                    label="Wajib"
                    :color="theme.color"
                    v-model="form.record.wajib"
                    :hide-details="device.desktop"
                  ></v-switch>
                </v-col>
                <v-col cols="4">
                  <v-switch
                    :label="record.status ? 'AKTIF': 'NON AKTIF'"
                    :color="theme.color"
                    v-model="form.record.status"
                    :hide-details="device.desktop"
                  ></v-switch>
                </v-col>
              </v-row>
...
export default {
  ...
  data: () => ({
    ...
    headers: [
      ...
      { text: "WAJIB", value: "wajib", align:'center', sortable:false, width:100 },
  ...
  created() {
    this.setPage({
      crud: true,
      dataUrl: "api/master-data/document-persyaratan/" + this.$route.params.jenis_dokumen_id,
      pagination: false,
    })
    this.fetchRecords();
  ...
  methods: {
    ...
    editRecord: async function(val) {
      ...
      this.form.record.dok = this.record.dok
      this.form.record.wajib = this.record.wajib
      this.form.record.status = this.record.status
    ...
    uploadFile: function () {
      this.assignFileBrowse({
        ...
        callback: (response) => {
          setTimeout(() => {
            this.form.record.dok = response.name;
          }, 500);
        },
      });
    },

<template>
  <v-row>
    <v-col cols="12">
      ...
      >
        <v-container>
          <div class="lokasi-title">DAFTAR HOTEL DI KABUPATEN TANGERANG</div>
          <div>
                ...
                <v-row>
                  <v-col :cols="device.desktop ? 6:12">
                    <div class="m-3">
                      ...
                      <div
                        class="mt-3"
                        v-show="record.website "
                      >
                        <v-btn
                          outlined
                          dense
                          color="blue"
                          v-show="record.website !== '' ? true : false"
                          @click="openWebsite"
                        >
                          Kunjungi Website
                        </v-btn>
...
export default {
  ...
  methods: {
    ...
    fetchLokasi: async function () {
      try {
        let { data } = await this.http.get("api/daftar-hotel");
        this.records = data;
        setTimeout(function () {
    ...
    postShowLokasi: async function (val) {
      try {
        let { data } = await this.http.get("api/show-hotel/" + val);
        this.record = data;
    ...
    openWebsite: function () {
      window.open(this.record.website, "__BLANK");
    },
  },
};
</script>

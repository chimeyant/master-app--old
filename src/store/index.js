...
export default new Vuex.Store({
  ...
  actions: {
    ...
    fetchRecords: async function({ state, commit, dispatch }, payload) {
      try {
        if (state.page.pagination) {
          commit("LOADING_MUTATION", { dialog: true, text: "Loading Data..." });
          commit("LOADING_MUTATION", {
            table: true,
          });
        ...
        commit("LOADING_MUTATION", { dialog: false, text: "" });
        commit("LOADING_MUTATION", {
          table: false,
        });
      }
     ...
     */
    setRecords: function({ commit, state }, payload) {},
    ...
    catchError: function({ commit, dispatch }, payload) {
      ...
      if (payload.hasOwnProperty("response") && payload.response) {
        ...
        if (status === 500) {
          commit("SNACKBAR_MUTATION", {
            color: "error",
            text: "Opps..., terjadi kesalahan " + response.message,
            state: true,
          });
        }
        if (status === 501) {
